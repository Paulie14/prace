...
\subsection{Saddle Point Weak Form}
...
\begin{subequations} \label{eqn:prob_mixed_weak_simple_discrete}
\begin{align}
...
Finally, we need to choose a~partition of unity. Since the hybridization decoupled the elements,
...
% 
% \notePE{
%     Instead it might be sufficient to prove one of the weaker conditions:
%     \begin{eqnarray}
...
% \notePE{We can do the LBB numerically but we must somehow show that the form $a_h$
...
The numerical validation is done at the discrete level, inspecting the eigenvalues of an~associated eigenvalue problem.
...
However, we are not sure about the relationship between $\eta_{min}$ and $\mathring\eta_{min}$.
...
\begin{figure}[!htb]
    ...
    \includegraphics[width=0.482\textwidth]{\results mh_dirichlet_vel.pdf} }
    %\hspace{5pt}
    \subfloat[$L_2$ elementwise error in velocity]{
    \includegraphics[width=0.482\textwidth]{\results mh_dirichlet_vel_error.pdf} }
...
\vspace{1cm}
The convergence results for both cases are gathered in Table \ref{tab:convergence_mh_dirichlet}.
...
\toprule
\multicolumn{2}{c|}{} & \multicolumn{2}{c|}{ SGFEM, $f=0$} & \multicolumn{2}{c|}{SGFEM} & \multicolumn{2}{c}{MHFEM}\\ [3pt] %\midrule
...
\begin{graph}[!htb]
    \centering    
    \includegraphics[width=0.8\textwidth]{\results mh_dirichlet_inf-sup.pdf} 
...
Finally, we present the inf-sup test results in \gref{graph:mh_dirichlet_inf-sup} according to \eqref{eqn:infsup_singular_values_hybrid}.
