...
This approach smears the local effects of the disruptions but the model can still provide a valid approximation for an initial global view.
...
The concept is also suitable due to the lack of detailed data which we suffer from in our applications nearly always.
...
but also larger effects (fractures) can be modelled by equivalent continuum.
...
but they cannot capture the local effects properly.
...
In this chapter we focus on the reduced dimensional concept which is being used in different fields
for many years. Most attention to this concept is given 
We are mainly interested in underground processes and large scale models, in particular 
the hydrogeological models of fractured porous media.
