...
\begin{eqnarray*}
    ...
    \textrm{element faces }    & F_{d}^{i}, & \quad i \in\mathcal{I}_{d,F} = \{1,\dots,N_{d,F} \} \\
    \textrm{element nodes }    & \vc x_{i}, & \quad i \in\mathcal{I}_{d,N} = \{1,\dots,N_{d,N} \}
% \end{align}
\end{eqnarray*}
...
Especially in Section \ref{sec:model_aquifer}, the quadrilateral elements are considered in contrast to the rest of the work
...
\begin{equation}
\int_{\Gamma^m_w} \{q\}^m_w \dd s = \int_{\Gamma^m_w} q - \avg{q}^m_w \dd s = 0.
\end{equation}
...
\div\, \vc u_1 &= \delta_1 f_1 + {\color{blue}\sum_{m\in\mathcal{M}}\abs{\Gamma^m_w}\Sigma^m_w\delta(\vc x - \vc x^m_w)} && \textrm{in } \Omega^w_1,\;\forall w\in\mathcal{W} \label{eqn:problem_2d_1d_source}\\
