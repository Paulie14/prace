...
\begin{align}
    a(\vc u_h,\vc v_h) + b(\vc v_h, p_h) &= \langle G, \vc v_h\rangle_{V'\times V} &&
        \forall \vc v_h\in \tilde V_h^{reg}, \label{eqn:mixed_saddle_discrete1}\\
...
If we take the equation \eqref{eqn:mixed_form_1} on a~single element $T^i$ (let it be an interior element not intersecting well for simplicity),
...
%     = \int_{E_j\setminus \Omega^w_C} \vc\psi_j\cdot\vc n \dd s
...
Find $\vc u=[\vc u_d] \in V_h$ and $p = [p_d, \lambda_d, \lambda^m_w] \in Q_h \times \Lambda_h$, 
$d=1,2,\, m\in\mathcal{M},\, w\in\mathcal{W}$ which satisfy
\begin{subequations}
\label{eqn:prob_saddle_coupled_12d}
    \begin{align}
    a_h(\vc{u},\vc v) + b_h(\vc v, p) &= \langle G, \vc v\rangle_{V_h'\times V_h} &&
        \forall \vc v\in V_h,
        \label{eqn:prob_saddle_coupled_12d_1}\\
    b_h(\vc{u}, q) -c_w(p,q) &= \langle F, q \rangle_{Q_h'\times Q_h} &&
        \forall q \in Q_h \times \Lambda_h
        \label{eqn:prob_saddle_coupled_12d_2}
    \end{align}
\end{subequations}
where the forms are
\begin{align*}
a_h(\vc u, \vc v)=& \sum_{\substack{d=1,2 \\ i\in \mathcal I_{dE}}} \int_{T_d^i}
   \delta_d^{-1} \vc u_d \vc K_d^{-1} \vc v_d \dd\bx, \\
%
b_h(\vc v, p)=&\sum_{\substack{d=1,2 \\ i\in \mathcal I_{dE}}}\left[
...
%
\color{blue}
 c_w(p,q) =& \color{blue}{ \sum_{\substack{w\in \mathcal{W} \\ m\in\mathcal{M}}} \int_{\Gamma^m_w} \delta_2(\bx^m_w)\sigma^m_w
    \big(p_1(\bx^m_w)-\lambda_w\big) \big(q_1(\bx^m_w)-\mu_w\big) \dd s},\\
 \langle G, \vc \psi\rangle_{V_h'\times V_h} =& \sum_{d=1,2}\sum_{i\in \mathcal T_d}
        \int_{\prtl T_d^i\cap \Gamma_{dD}}
                 - g_{dD} (\vc \psi_d \cdot \vc n) \dd s,\\
 \langle F, q\rangle_{Q_h'\times Q_h} =& \sum_{d=1,2}\sum_{i\in \mathcal T_d}%\left(
...
We note that the nonzero Neumann boundary condition is also implemented and it
...
Find $\vc u=[\vc u_d] \in V_h$ and $p = [p_d, \lambda_d, \lambda_w] \in Q_h \times \Lambda_h$, 
...
where the forms are
\begin{align*}
a_h(\vc u, \vc v)=& \sum_{\substack{d=1,3 \\ i\in \mathcal I_{dE}}} \int_{T_d^i}
   \delta_d^{-1} \vc u_d \vc K_d^{-1} \vc v_d \dd\bx, \\
%
b_h(\vc v, p)=&\sum_{\substack{d=1,3 \\ i\in \mathcal I_{dE}}}\left[
        \int_{T_d^i} -p_d\,\div\,\vc v_d \dd\bx, 
        + \int_{F^i_d\setminus \prtl\Omega}
                 \lambda_d (\vc v_d \cdot \vc n) \dd s\right]\\
...
%
\color{blue}
 c_w(p,q) =& \color{blue}{ \sum_{w\in \mathcal{W}}
        \Bigg[ 2\pi\rho_w\abs{\Omega^w_1} \sum_{\substack{i\in\mathcal{I}_{1E} \\ T^i_1\subset\Omega^w_1}}
        \int_{T^i_1} \sigma_w \big(p_1(t)-\lambda_w\big) \big(q_1(t)-\mu_w\big) \dd t} \Bigg],\\
 \langle G, \vc \psi\rangle_{V_h'\times V_h} =& \sum_{d=1,3}\sum_{i\in \mathcal T_d}
        \int_{\prtl T_d^i\cap \Gamma_{dD}}
                 - g_{dD} (\vc \psi_d \cdot \vc n) \dd s,\\
 \langle F, q\rangle_{Q_h'\times Q_h} =& \sum_{d=1,3}\sum_{i\in \mathcal T_d}%\left(
        \int_{T_d^i} - \delta_d f_d q_d \dd\bx.
%         - \int_{\prtl T_d^i\cap \Gamma_{dN}}
%                  g_{dN} \mu_d\right),
\end{align*}
