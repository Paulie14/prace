...
a large difference in the size of neighbouring subelements, see \fig{fig:adapt_ref_a}. Although
...
cumulating outside the smaller enrichment zones, in case of larger enrichment radii only the error of the regular part is apparent.
%
\begin{table}[!htb]
\begin{center}
\bgroup
...
We do not mention the results of shifted XFEM explicitely since it performs nearly the same as SGFEM with the smaller enrichment radius.
...
The well-aquifer model was defined in the begining and its weak form was derived, including the analysis of the weak solution existence.
Then the discretization by means of the XFEM was thoroughly described. The range of the numerical tests was narrowed
to problems with a~single aquifer for which the pseudo-analytic solution derived.
...
The shifted performed well when there was only one singular enrichment per element. Otherwise, when the enrichment
zones from multiple singularities were overlapping, only the SGFEM behaved well in terms of system matrix conditioning.

A proper size of the enrichment area, defined by the enrichement radius, was studied. The error estimate dependent
...
cross-section. The precise analytic solution was derived and the range of the numerical tests was broaden.
