...
We look for some clues for a~particular choice of $R_w$ that is optimal in a~specific sense.
...
\begin{equation}
    ...
    \norm{p - p_h}_{H^1(\Omega)} \le c_a \big(c h \abs{p_{reg}}_{H^2(\Omega)} + \norm{p_{sin} - \Pi p_{sin}}_{H^1(Z'_w)} \big)
\end{equation}
where $\Pi p_{sin}$ denotes interpolation of $p_{sin}$ in $V^{reg}_h$. Our next aim is to find tight estimate for the second term.
...
\begin{align*}
 (p_{sin} - \Pi p_{sin})(s,t)&=\log\sqrt{(r+hs)^2+(ht)^2} -\Big[(1-s)(1-t)\log r\\
...
and 
\begin{equation}
 \grad(p_{sin} - \Pi p_{sin})(s,t) = \frac{h}{r^2} \left( \frac12-s, t-\frac12 \right) + O\left(\frac{h^2}{r^2}\right).
\end{equation}
Assuming $h<r$, we can neglect higher order terms. Then, we obtain by direct integration
\begin{align*}
 \norm{p_{sin} - \Pi p_{sin}}^2_{L^2(S_{h,r})} \approx \frac14 \frac{h^6}{r^4}\int_0^1\int_0^1 \left(t^2-t-s^2+s\right)^2\,\dd s\, \dd t = \frac{1}{360}\frac{h^6}{r^4} 
\end{align*}
and
\begin{equation}
    \label{eq:grad_estimate_on_square}
    \norm{\grad(p_{sin} - \Pi p_{sin})}^2_{L^2(S_{h,r})} \approx \frac{2h^4}{r^4} \int_0^1 \Big(t-\frac12\Big)^2 \dd t = \frac{1}{6}\frac{h^4}{r^4}.
\end{equation}
Thus for the density of squared error we have
\[
    \frac{1}{\abs{S_{h,r}}} \norm{p_{sin} - \Pi p_{sin}}^2_{H^1(S_{h,r})} \approx \frac{h^2}{6r^4}
\]
which after integration over the unenriched domain gives final estimate:
\begin{equation}
    \label{eq:singular_approx_error}
    \norm{p_{sin} - \Pi p_{sin}}_{H^1(Z'_w)} \le \left[\int_0^{2\pi} \int_{R_w}^\infty \frac{h^2} {6r^4} r \,\dd r\, \dd \theta\right]^{1/2} = \sqrt\frac{\pi}{6}\frac{h}{R_w}. 
\end{equation}

Recalling the estimate \eqref{eq:std_err_estimate}, we can conclude that optimal choice of the enrichment radius is $h/R_w\approx \norm{p_{reg}-\Pi p_{reg}}_{H^1(\Omega)}$, 
which balances the error in the regular and the singular part. In combination with an a~posteriori error analysis, this could give a~rule for an automatic
...
Let us now define four test cases on which we investigate the behaviour of the methods, mainly their convergence properties.
...
We alert the reader that the converegence graphs of ramp function XFEM, shifted XFEM and SGFEM overlap in \fig{fig:convergence01}.
...
%
% TODO: rename FEM (ur) to FEM_{reg}
\begin{figure}[!htb]
%   \vspace{0pt}
  \centering    
...
To explain this behaviour we plot the error distribution in two different refinement levels in \fig{fig:error_distribution_test4}.
...
\end{figure}
%
\notePE{Not sure if it is proper to include following results in \fig{fig:moving_xw_error} and \fig{fig:moving_xw_iterations}.}
...
The aim of this section is twofold: we first validate the estimate \eqref{eq:grad_estimate_on_square}, secondly we simulate 
...
\frac{h^{3/2} r^{-2} 12^{-1/2}}{\|p_{sin} - \Pi p_{sin}\|^2_{H^1(T)}},\quad p_{sin}(\vc x) = \log \abs{\vc x}
\end{equation}
...
Next, we study the influence of the enrichment radius $R_w$ on the global $L^2$ error. To this end, we solve Problem 2 with parameter $U=3$
using the shifted XFEM for different mesh steps and different values of $R_w$.
...
\[
    R_o \sim \sqrt{\frac{\pi}{6}} h/\norm{p_{reg} - \Pi p_{reg}}_{H^1(\Omega)} \sim 0.36
\]
