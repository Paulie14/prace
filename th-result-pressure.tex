...
% In the definition of $V^m_0$ in \eqref{eqn:prim_weak_space_Vm0}, we assume that the wells are very small in diameter,
% $\rho_w << |\Omega^m_2|$, and that pressure is changing minimally along $\Gamma^m_w$.
% Thus we effectively neglect the given boundary condition for the pressure fluctuation in \eqref{eqn:prim_problem_2d_press_fluct}.
% and we put
...
We now prove that the bilinear form $a$ is coercive
\begin{equation} \label{eqn:prim_form_a_coercivity_V0}
    a(q,q) \geq \alpha_1\norm{q}^2_V \quad \forall q\in V_0.
\end{equation}
