...
% we look deeper into this topic and from a~more general perspective.
...
% otherwise it can be troublesome or lead to large computational costs.
...
Then we discuss our implementation of the advancing front technique and the usage of AABB and BIH for its initialization.
...
\begin{itemize}
 \item the barycentric coordinate $\vc w_K$ of IC on element $K$,
 \item the dimension $d_K$ of the lowest dimensional \nface the IC is lying on, \\
    e.g. an IC on an edge have $d_K=1$ although it also lies o
    ktdds14
    n the connected faces,
...
\begin{figure}[!htb]
    ...
    {Different relative positions of a~ line $p$ and a~triangle with sides $s_i$, $i=0,1,2$. 
        The dashed parts of $p$ are hiiden behind the triangle. The signs of the permuted inner products depend on the orientation of lines,
...
All the possible intersection of $p$ and $S_2$ is displayed in \fig{fig:1d2d_cases}.
...
\begin{enumerate}
    ...
    \item If two $w_i$ are less than $\epsilon$, the intersection is at the vertex $V_i$, we set $\tau_{S_2}=(0,i)$. \\
...
In order to make the test in the step \ref{item:zero_total} independent of the  scale of the elements, we use the characteristic lengths $L_1$ and $L_2$ of $S_1$ and $S_2$ respectively.
Further, the algorithm depends on the parameter $\epsilon$ which is used as a~common tolerance parameter for detection of ICs with special positions. First, it is used 
in the sign check for the permuted inner products, second, it is used for the topological check on the line. Only these two kinds of geometric predicates are used 
through the all intersection algorithms. Currently, we use a~fixed value $\epsilon=10^{-9}$. This value is close to the machine epsilon ($10^{-16}$ 
...
Let us now consider an intersection of a~line segment $S_1$, defined by a~parameter $t\in [0,1]$ of the line $p$ defined in \eqref{eq:line_parametric}, with a~tetrahedron 
$S_3$. The algorithm is based on the 1d-2d algorithm and closely follows \cite{platis_fast_2003}. Our modification takes into account
intersection with the line segment and consistently propagates topological positions of ICs.
...
\begin{algorithm}
  ...
  \For{unmarked face $f$ of $S_3$}{
    ...
    \lIf{ $L$ is none or degenerate}{\continue} \label{line:13-noIC}
...
The intersection corners are appended to the list $I$ as they are computed, however their order in IP is 
...
After an IC $p$ is computed, we also obtain two $n$-faces $x$, $y$ incident with the two IP's sides that are connected by the IC. 
...
To this end we call SL with $e$ as the target which allows to use backlink and get the already computed IC if it is rediscovered later on.
...
Special action must be taken in degenerate cases with less then $3$ ICs, since they may not form a~cycle.
...
If this step is done only a~few times the optimal way it to iterate overt the bulk mesh and test every element for the intersection.
...
of the mesh intersection algorithm is shown in \gref{graph:cube_speed}. Every algorithm consists of the initialization phase which process all elements of the mesh 
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.43\textwidth]{\figpathins test1_cube0.pdf}
...
\begin{graph}[!htb]
    \centering
    \includegraphics[width=1.0\textwidth]{\figpathins cube_scale.pdf}
...
\begin{figure}[!htb]
    \subfloat[interior fractures]{
        \includegraphics[width=0.49\textwidth]{\figpathins bedrichov_fract.pdf}
        \label{fig:bedrichov_cut}
    }
    \hspace{3pt}
    \subfloat[extending fractures]{
        \includegraphics[width=0.49\textwidth]{\figpathins bedrichov_fract_extend.pdf}
