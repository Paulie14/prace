...
In this test case we want to demonstrate more general setting including 10 wells of different tilt.
...
Instead of setting pressure in the wells, we prescribe fluxes by the means of the inhomogenous Neumann boundary condition
at the top of the wells. The fluxes are gathered in the first row of Table \ref{tab:tc7_data}, the second row
contents the fluxes scaled by the cross-section, i.e. flux density for Flow123d input.
The rock block conductivity is set to $10^{-2}$, Dirichlet boundary condition $g_{3D}=25(3-z)$
is applied on the sides and homogenous Neumann boundary condition is set at the bases.
%
\begin{table}[!htb]
\begin{center}
...
$g^w_{1N}$ & 1.414 & 0.848 & 0.565 & 1.414 & 1.131 & -1.131 & 0.848 & 0.848 & -1.131 & 0.565\\
$g^w_{1N} / \delta_1$ & 500  & 300 & 200 & 500 & 400 & -400 & 300 & 300 & -400 & 200\\
\bottomrule
\end{tabular}
...
% \begin{figure}[!htb]
%     \centering
%     \includegraphics[width=0.8\textwidth]{\results tc_30_10w.pdf}
...
block sides is -5.376, i.e. the difference is between what flows into and out of the system is 0.004.
...
\section{Summary}
In this chapter we developed an XFEM in mixed-hybrid form for approximation of singularities for both 2d and 3d problems.
The model was formulated following the concept of the software Flow123d and using the Lagrange multipliers for dimnesional coupling.
...
The numerical validation of the inf-sup stability was discussed, however this topic if left open for further research.
...
The models were implemented as a~part of the software Flow123d.
...
Although pressure converges suboptimally, since its FE approximation is not enriched, velocity error
